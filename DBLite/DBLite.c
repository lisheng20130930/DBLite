...
{
	int t = strlen(buffer+*pos)+1;
	char *v = we_cmmn_strndup(buffer+*pos,t);
	*pos+=t;
	return v;
}
...
{
	int t = strlen(v)+1;
	extendBuffer(buffer,len,*pos,t);
	memcpy(*buffer+*pos,(void*)v,t);
	*pos+=t;
}
...
{
	int t = 0;
	void *v = buffer2Obj(BJ[oIdx].pFieldTable,BJ[oIdx].oSize,buffer+*pos,len-*pos,&t);
	*pos+=t;
	return v;
}
...
static void writeObject(char **buffer,int *len, int *pos, void *v, int oIdx)
{
	...
	int iTmpLen = 0;
	char *pTmpBuffer = obj2Buffer(BJ[oIdx].pFieldTable,v,&iTmpLen);
	if(!pTmpBuffer){
		return;
	}
	
	extendBuffer(buffer,len,*pos,iTmpLen);
	memcpy(*buffer+*pos,(void*)pTmpBuffer,iTmpLen);
	*pos+=iTmpLen;
